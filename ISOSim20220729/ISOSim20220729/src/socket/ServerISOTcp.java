...
import com.megatus.megaengine.listener.tcp.NioTcpListener;
...
public class ServerISOTcp {
	...
	private static volatile ServerISOTcp instance = null;
	...
	public static ServerISOTcp getInstance()
	{
		if (instance == null) {
			synchronized (ServerISOTcp.class) {
				if (instance == null) {
					instance = new ServerISOTcp();
				}
			}
		}
		return instance;
	...
	public static void main(String[] args) throws IOException {
		...
		LOG.warning("Server siap menerima koneksi pada port ["+PORT+"]");
		LOG.warning("Mulai");
		...
		while(true) {
			...
			LOG.warning("\r");
			LOG.warning("\r");
			LOG.warning("Client IP Address: " + clientIpAddress);
			LOG.warning("Client Port: " + clientPort);
			...
			while((data = inStreamReader.read()) != 0) {
				...
				if (lengthOfMsg == sb.toString().length()) {
					LOG.warning("Rec. Msg ["+sb.toString()+"] len ["+sb.toString().length()+"]");
